...
def find_example_file(name: str, base_dir: Path) -> Path:
    ...
    Args:
        name: Example name (e.g., "belova_case1_resistive")
...
def list_examples(base_dir: Path) -> list:
    ...
    for yaml_file in base_dir.glob("*.yaml"):
        examples.append(yaml_file.stem)
...
Examples:
  ...
  %(prog)s belova_case1_resistive    Run specific example
  %(prog)s --all                     Run all examples
    ...
    if args.all:
        for example in list_examples(base_dir):
            example_files.append(base_dir / f"{example}.yaml")
    else:
