"""Belova Case 1: Large FRC Merging Without Compression (Qualitative)

Physics:
    This test validates qualitative behavior for FRC (Field-Reversed Configuration)
    merging simulations based on Belova et al. The simulation models counter-helicity
    merger of two large FRC plasmoids approaching from opposite ends of a flux
    conserver.

    Key physics:
        - Two FRCs with opposite toroidal field (counter-helicity)
        - Initial separation dZ = 180 (normalized) with axial velocity Vz = 0.2 vA
        - Resistive MHD with Chodura anomalous resistivity
        - Conducting wall boundary conditions

    FRC parameters (Belova et al. Fig. 1-2):
    ...
    Expected outcome:
        Partial merge forming a doublet configuration. The large FRCs do not fully
        merge without external compression, but instead form a tilted doublet state
        with residual separatrix between the two plasmoids.
...
from jax_frc.configurations.frc_merging import BelovaCase1Configuration
...
def setup_configuration(quick_test: bool = False) -> dict:
    ...
    return {
        ...
        "nr": 64,
        "nz": 128,
        # For testing, use smaller nz. Production would use 512.
    }
