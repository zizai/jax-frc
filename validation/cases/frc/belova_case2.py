"""Belova Case 2: Small FRC Merging Without Compression (Qualitative)

Physics:
    This test validates qualitative behavior for small FRC (Field-Reversed Configuration)
    merging simulations based on Belova et al. The simulation models counter-helicity
    merger of two small FRC plasmoids approaching from opposite ends of a flux
    conserver.

    Key physics:
        - Two FRCs with opposite toroidal field (counter-helicity)
        - Initial separation dZ = 75 (normalized) with axial velocity Vz = 0.1 vA
        - Resistive MHD with Chodura anomalous resistivity
        - Conducting wall boundary conditions

    FRC parameters (Belova et al. Fig. 3-4):
        S* = 20 (kinetic parameter)
        E = 1.5 (elongation)
        xs = 0.53 (normalized separatrix radius)
        beta_s = 0.2 (separatrix beta)

    Expected outcome:
        Complete merge by ~5-7 tA. The smaller FRCs merge more readily than
        the large FRCs in Case 1, forming a single merged FRC configuration.

    This is a QUALITATIVE validation - there is no analytic solution. We verify:
        - Numerical stability (no NaN/Inf)
        - Flux conservation within tolerances
        - (Optional) Merge completion check
...
# Case metadata
NAME = "belova_case2"
DESCRIPTION = "Small FRC merging without compression (Belova et al. Fig. 3-4)"
...
ACCEPTANCE = {
    ...
    "flux_conservation": {
        "threshold": 0.1,  # 10% drift allowed
...
def main(quick_test: bool = False) -> bool:
    ...
    overall_pass = stability_pass and flux_pass
