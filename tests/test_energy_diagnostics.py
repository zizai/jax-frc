...
class TestEnergyDiagnostics:
    ...
    def zero_state(self, geometry):
        ...
        return State.zeros(nx=geometry.nx, ny=geometry.ny, nz=geometry.nz)

    @pytest.fixture
    def nonzero_B_state(self, geometry):
        """Create a state with nonzero magnetic field."""
        state = State.zeros(nx=geometry.nx, ny=geometry.ny, nz=geometry.nz)
    ...
    def nonzero_p_state(self, geometry):
        ...
        p = jnp.ones((geometry.nx, geometry.ny, geometry.nz)) * 1000.0
    ...
    def test_total_equals_sum_of_components(self, geometry):
        ...
        v = jnp.zeros((geometry.nx, geometry.ny, geometry.nz, 3))
        v = v.at[:, :, :, 0].set(500.0)  # v_x = 500 m/s

        n = jnp.ones((geometry.nx, geometry.ny, geometry.nz)) * 1e19
        p = jnp.ones((geometry.nx, geometry.ny, geometry.nz)) * 500.0
    ...
    def test_magnetic_energy_scales_with_B_squared(self, geometry):
        ...
        state1 = State.zeros(nx=geometry.nx, ny=geometry.ny, nz=geometry.nz)
        state2 = State.zeros(nx=geometry.nx, ny=geometry.ny, nz=geometry.nz)
    ...
    def test_kinetic_energy_scales_with_v_squared(self, geometry):
        ...
        v1 = jnp.zeros((geometry.nx, geometry.ny, geometry.nz, 3))
        v1 = v1.at[:, :, :, 2].set(1000.0)

        v2 = jnp.zeros((geometry.nx, geometry.ny, geometry.nz, 3))
    ...
    def test_thermal_energy_scales_with_pressure(self, geometry):
        ...
        p1 = jnp.ones((geometry.nx, geometry.ny, geometry.nz)) * 1000.0
        p2 = jnp.ones((geometry.nx, geometry.ny, geometry.nz)) * 2000.0  # Double pressure
    ...
    def test_kinetic_energy_scales_with_density(self, geometry):
        ...
        n1 = jnp.ones((geometry.nx, geometry.ny, geometry.nz)) * 1e20
        n2 = jnp.ones((geometry.nx, geometry.ny, geometry.nz)) * 2e20  # Double density
    ...
    def test_magnetic_energy_includes_all_components(self, geometry):
        ...
        # Set only B_r
        B_r = jnp.zeros((geometry.nx, geometry.ny, geometry.nz, 3))
        B_r = B_r.at[:, :, :, 0].set(1.0)

        # Set only B_phi
        B_phi = jnp.zeros((geometry.nx, geometry.ny, geometry.nz, 3))
        B_phi = B_phi.at[:, :, :, 1].set(1.0)

        # Set only B_z
        B_z = jnp.zeros((geometry.nx, geometry.ny, geometry.nz, 3))
    ...
    def test_kinetic_energy_includes_all_components(self, geometry):
        ...
        # Set only v_r
        v_r = jnp.zeros((geometry.nx, geometry.ny, geometry.nz, 3))
        v_r = v_r.at[:, :, :, 0].set(1000.0)

        # Set only v_phi
        v_phi = jnp.zeros((geometry.nx, geometry.ny, geometry.nz, 3))
        v_phi = v_phi.at[:, :, :, 1].set(1000.0)

        # Set only v_z
        v_z = jnp.zeros((geometry.nx, geometry.ny, geometry.nz, 3))
