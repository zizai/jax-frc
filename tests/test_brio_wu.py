"""Tests for BrioWuConfiguration."""
import pytest
import jax.numpy as jnp


def test_brio_wu_builds_geometry():
    """Configuration creates Cartesian geometry."""
    from jax_frc.configurations import BrioWuConfiguration
...
def test_brio_wu_initial_conditions():
    ...
    # Left state (z < 0.5): rho=1.0, p=1.0
...
def test_brio_wu_magnetic_field():
    """B field has Bz=0.75 constant, By reverses."""
    from jax_frc.configurations import BrioWuConfiguration
    ...
    # By should be +1 on left, -1 on right
    left_idx = geometry.nz // 4
    right_idx = 3 * geometry.nz // 4
    x_idx = geometry.nx // 2
    y_idx = geometry.ny // 2
    assert state.B[x_idx, y_idx, left_idx, 1] > 0.5  # By > 0 on left
    assert state.B[x_idx, y_idx, right_idx, 1] < -0.5  # By < 0 on right


def test_brio_wu_builds_model():
